...
Usage:
  ...
  python distribb_cli.py admin:projects:list --q acme --gsc connected                  # ADMIN: find any customer's project
  python distribb_cli.py admin:search-console --project-id 575 --days 90 --compare     # ADMIN: read any project's Search Console
...
def cmd_admin_landers_create(args):
    data = _admin_lander_body(args)
    data['slug'] = args.slug
    if getattr(args, 'serve_at', None):
        data['serve_at'] = args.serve_at
    if not data.get('title') or not data.get('content'):
        print(json.dumps({"error": "Creating a page needs --title and --content/--content-file."}))
        sys.exit(1)
    print(json.dumps(api('POST', '/api/v1/admin/landers', json_data=data), indent=2))
...
def cmd_admin_projects_list(args):
    params = {}
    for flag in ('q', 'user', 'status', 'gsc', 'limit', 'offset'):
        value = getattr(args, flag, None)
        if value is not None:
            params[flag] = value
    print(json.dumps(api('GET', '/api/v1/admin/projects', params=params), indent=2))


def cmd_admin_search_console(args):
    if not args.project_id and not args.domain:
        print(json.dumps({"error": "--project-id or --domain is required (find it with admin:projects:list)"}))
        sys.exit(1)
    params = {}
    if args.project_id:
    ...
    for flag in ('days', 'limit', 'start_row', 'dimensions', 'query_contains', 'page_contains', 'search_type'):
        value = getattr(args, flag, None)
        if value is not None:
            params[flag] = value
    if args.compare:
        params['compare'] = 1
    print(json.dumps(api('GET', '/api/v1/admin/search-console', params=params), indent=2))
...
def main():
    ...
    p.add_argument('--project-id', type=str, help="Project id, or 'null' for the legacy rows that predate the project system")
    p.add_argument('--include-legacy', action='store_true', help='Also return rows with no project (use with --project-id 37 to get the whole distribb.io blog)')
    ...
    p = sub.add_parser('admin:landers:create', help='ADMIN: create a landing page. Live at https://distribb.io/<slug> immediately, no deploy.')
    p.add_argument('--slug', type=str, required=True,
                   help="Bare page name in lowercase letters, digits and hyphens ('seo-for-dentists'). "
                        "No slashes and no /lp/ prefix: the page serves at the root URL.")
    ...
    p.add_argument('--meta-description', type=str)
    p.add_argument('--meta-keywords', type=str)
    p.add_argument('--canonical-url', type=str, help='Defaults to the live URL')
    p.add_argument('--schema', type=str, help='JSON-LD string. Rejected unless it parses as JSON.')
    p.add_argument('--schema-file', type=str, help='Path to a JSON-LD file')
    p.add_argument('--meta-image', type=str, help='Absolute URL for og:image / twitter:image')
    p.add_argument('--serve-at', type=str, choices=['root', 'lp'],
                   help="Where the page lives. Default 'root' (/<slug>); 'lp' keeps the legacy /lp/ prefix.")
    p.add_argument('--json', type=str, help='Raw JSON body for full control')
    p.set_defaults(func=cmd_admin_landers_create)

    p = sub.add_parser('admin:projects:list', help="ADMIN: find any customer's project and see whether Search Console is connected")
    p.add_argument('--q', type=str, help='Business name, website or owner email contains')
    p.add_argument('--user', type=str, help='Owner email (exact)')
    p.add_argument('--status', type=str, help='Project status such as Active or Paused (default: any)')
    p.add_argument('--gsc', type=str, choices=['connected', 'missing'], help='Only projects with (or without) a live Search Console connection')
    p.add_argument('--limit', type=int, help='Default 50, max 200')
    p.add_argument('--offset', type=int)
    p.set_defaults(func=cmd_admin_projects_list)

    p = sub.add_parser('admin:search-console', help="ADMIN: read ANY project's Google Search Console performance (read-only)")
    p.add_argument('--project-id', type=int, help='Project id (see admin:projects:list)')
    p.add_argument('--domain', type=str, help='Find the project by website instead, e.g. acme.com')
    p.add_argument('--days', type=int, help='Lookback window in days (default 28, max 90)')
    p.add_argument('--limit', type=int, help='Rows per list (default 25, max 1000)')
    p.add_argument('--start-row', type=int, help='Paging offset into the Search Console rows')
    p.add_argument('--compare', action='store_true', help='Add period-over-period deltas against the preceding window of equal length')
    p.add_argument('--dimensions', type=str, help='Comma list from query,page,date,country,device,searchAppearance. Returns rows keyed by them (query,page = which page ranks for which query)')
    p.add_argument('--query-contains', type=str, help='Only rows whose query contains this text')
    p.add_argument('--page-contains', type=str, help='Only rows whose page URL contains this text')
    p.add_argument('--search-type', type=str, choices=['web', 'image', 'video', 'news', 'discover', 'googleNews'])
    p.set_defaults(func=cmd_admin_search_console)
